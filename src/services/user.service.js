...
class UserServices {
  async addServer(name, service, unit, fabId, roomId, rackId) {
    ...
    try {
      ...
      const ip = await ipService.assign(fabId, service);
      ...
      const server = await serverService.createServer(name, service, ip, unit, fabId, roomId, rackId, frontPosition, backPosition);
