...
class FabServices {
  ...
  async getFab(id) {
    ...
    const query = ` 
      SELECT 
        dc.id AS dc_id, dc.name AS dc_name,createdAt,updatedAt
        r.id AS room_id, r.name AS room_name,
        rk.id AS rack_id, rk.name AS rack_name, rk.service,
        s.id AS server_id, s.name AS server_name,
      FROM fabs dc
      JOIN rooms r ON r.fabId = dc.id
      JOIN racks rk ON rk.roomId = r.id
      JOIN servers s ON s.rackId = rk.id
      WHERE dc.id = $1
  ...
  async getAllFabs() {
    const query = ` 
      SELECT 
        ...
        r.id AS room_id, r.name AS room_name,
        rk.id AS rack_id, rk.name AS rack_name, rk.service,
        s.id AS server_id, s.name AS server_name, s.unit, s.position_front, s.position_back
      FROM fabs dc
      JOIN rooms r ON r.fabId = dc.id
      JOIN racks rk ON rk.roomId = r.id
      JOIN servers s ON s.rackId = rk.id
    ...
    for (const row of rows) {
      ...
      rack.servers[serverKey] = {
        name: row.server_name,
        unit: row.unit,
        position_front: row.position_front,
        position_back: row.position_back,
      };
    }
